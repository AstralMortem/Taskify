...
from taskify.api.deps import auth_required, get_board_service, get_user_service, HasPermission
from taskify.models.auth import User
from taskify.models.board import BoardMemberRel
from taskify.schemas.board import BoardCreate, BoardMemeber, BoardRead, BoardUpdate, PublicBoardUpdate
...
class BoardController(Controller):
    ...
    async def get_user_boards(
        self, params: Params = Depends(), user: User = Depends(HasPermission(Action.READ))
    ):
        return await self.service.get_user_boards(user.id, params)
    
    @Controller.get('/membered/all', response_model=Page[BoardRead])
    async def get_a_membered_board(self, params: Params = Depends(), user: User = Depends(HasPermission(Action.READ))):
    ...
    async def get_board(self, board_id: uuid.UUID, user: User = Depends(HasPermission(Action.READ))):
    ...
    async def update_publicity(self, board_id: uuid.UUID, is_public: PublicBoardUpdate , user: User = Depends(HasPermission(Action.UPDATE))):
        return await self.service.change_board_publicity(board_id, is_public.is_public, user)
