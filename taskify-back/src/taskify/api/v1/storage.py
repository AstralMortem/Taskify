...
from taskify.api.deps import auth_required, get_file_storage, StorageService
...
class FileStorageController(Controller):
    ...
    async def upload_avatar(self, file: UploadFile , user: User = Depends(HasPermission('avatar'))):
        return await self.service.upload_avatar(file, user)


    @Controller.put('/{object_key:path}', response_model=FileUploadResponse)
    async def upload_file(self, object_key: str, file:UploadFile, user = Depends(HasPermission('upload'))):
