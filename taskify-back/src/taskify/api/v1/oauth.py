...
github_oauth_client = GitHubOAuth2(settings.GITHUB_CLIENT_ID, settings.GITHUB_SECRET_KEY)


def generate_state_token(payload: dict[str, Any]):
    payload["aud"] = settings.JWT_STATE_TOKEN_AUDIENCE
    payload["iat"] = datetime.now(UTC)
    payload['exp'] = datetime.now(UTC) + timedelta(seconds=settings.JWT_STATE_TOKEN_MAX_AGE)
...
class OAuthController(Controller):
    ...
    async def github_login(self, request: Request):
        ...
        authorization_url = await github_oauth_client.get_authorization_url(
            redirect_uri=settings.GITHUB_CALLBACK_URL or str(request.url_for('github-callback')),
            state=state,
        )
        return OAuth2AuthorizeResponse(authorization_url=authorization_url)
    
    @Controller.get('/github/callback', name="github-callback")
    async def github_callback(self, request: Request, access_token_state: tuple[OAuth2Token, str] = Depends(OAuth2AuthorizeCallback(github_oauth_client, redirect_url=settings.GITHUB_CALLBACK_URL))):
        token, state = access_token_state
        return await self.service.oauth_callback(token, state, github_oauth_client)        
        
        
    @Controller.delete('/{oauth_id}', status_code=204)
    async def remove_oauth_account(self, oauth_id:uuid.UUID,  user: User = Depends(auth_required)):
