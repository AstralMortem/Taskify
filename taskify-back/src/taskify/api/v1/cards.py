...
class CardController(Controller):
    ...
    async def get_card(
        self, card_id: uuid.UUID, user=Depends(HasPermission(Action.READ))
    ):
    ...
    async def get_list_cards(
        ...
        user=Depends(HasPermission(Action.READ)),
    ):
        return await self.service.get_list_cards(list_id, params)

    @Controller.get(
        "/list/{list_id}/public/{board_hash}", response_model=Page[CardRead]
    )
    async def get_public_list_cards(
        self, list_id: uuid.UUID, board_hash: str, params: Params = Depends()
    ):
    ...
    async def create_card(
        ...
        list_id: uuid.UUID,
        payload: CardCreate,
        user=Depends(HasPermission(Action.CREATE)),
    ):
    ...
    async def delete_card(
        self, card_id: uuid.UUID, user=Depends(HasPermission(Action.DELETE))
    ):
