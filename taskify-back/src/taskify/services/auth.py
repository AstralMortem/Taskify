...
from taskify.utils import password_helper
...
class AuthService:
    ...
    async def reset_password(self, credentials: ResetPasswordCredentials, user: User):
        is_valid, _ = self.password_helper.verify_and_update(credentials.old_password, user.hashed_password)
        if not is_valid:
            raise TaskifyException(status.HTTP_400_BAD_REQUEST, "Incorrect password", "Your current password is incorect. Try again")
    ...
    async def forgot_password(self, email: str):
        ...
        if user is None:
            raise TaskifyException(status.HTTP_400_BAD_REQUEST, "User does not exits", "User with provided email does not exist")
        
        token = self._encode_jwt_token({"sub": user.email}, settings.JWT_RESET_TOKEN_MAX_AGE, settings.JWT_RESET_TOKEN_AUDIENCE)

        return token
    
    async def verify_forgotten_password(self, new_password: str, token: str):
        error = lambda e: TaskifyException(status.HTTP_400_BAD_REQUEST, "Invalid reset token", "Invalid reset token", debug=e)
        try:
            decoded_token = self._decode_jwt_token(token, settings.JWT_RESET_TOKEN_AUDIENCE)
    ...
    async def oauth_callback(self, token: OAuth2Token, state: str, client: BaseOAuth2):
        ...
        if account_email is None:
            raise TaskifyException(status.HTTP_400_BAD_REQUEST, "Invalid Login", "OAuth2 don`t provide email")
        
        try:
            jwt.decode(state, settings.JWT_SECRET_KEY, algorithms=[settings.JWT_ALGORITHM], audience=settings.JWT_STATE_TOKEN_AUDIENCE)
        except jwt.DecodeError as e:
            raise TaskifyException(status.HTTP_400_BAD_REQUEST, "Invalid token", "Can not decode state token", debug=e)
    ...
    async def remove_oauth(self, id: uuid.UUID, user: User):
        ...
        if account is None:
            raise TaskifyException(status.HTTP_404_NOT_FOUND, "OAuth Account not found", "OAuth account with such name does not exits")
        if account not in user.oauth_accounts:
            raise TaskifyException(status.HTTP_403_FORBIDDEN, "This account not associated with user", f"OAuth Account not asociated with user {user.email}")
        await self.oauth_repo.delete(account)

    def _create_login_response(self, user: User) -> Response:
        response = Response(status_code=204)
        token = self._encode_jwt_token(
            {'sub': str(user.id)}, settings.JWT_AUTH_TOKEN_MAX_AGE, settings.JWT_AUTH_TOKEN_AUDIENCE
        )
