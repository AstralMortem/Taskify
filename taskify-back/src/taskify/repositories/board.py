...
class BoardRepository(BaseRepository[Board, uuid.UUID]):
    ...
    async def get_membered_board(self, member: User, params: Params) -> Page[Board]:
        qs = (
            select(self.model)
            .join(BoardMemberRel, BoardMemberRel.board_id == self.model.id)
            .where(BoardMemberRel.user_id == member.id)
        )
    ...
    async def get_public_board(self, hash: str) -> Board | None:
        qs = (
            select(self.model)
            .where(self.model.public_hash == hash, self.model.is_public == True)
            .limit(1)
        )
